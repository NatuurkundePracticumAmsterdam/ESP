...
An important consequence of the logarithmic notation is that, when a coupling is made with a number of systems after each other, the overall gain can be found as the sum of the individual gain of each system expressed in decibels.
...
Although the decibel notation is only applicable to power amplification, in literature, it is very common to use it for voltage amplification as well, even when $R_{\rm L}$ and $R_{\rm i}$ are not equal to each other. The voltage gain in decibels is defined as:
